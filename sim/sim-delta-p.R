...
DIR <- 'plots/deltap/deltap-'

## Conjunctive
##  p(E) = p(C)*p(A)
##  p(E|C) = p(A)
##  p(E|-C) = 0
##  DP = p(E|C) - p(E|-C) = p(A) - 0 = p(A)
##
## Disjunctive
##  p(E) = p(C) + p(A) - p(C)*p(A)
##  p(E|C) = 1
##  p(E|-C) = p(A)
##  DP = p(E|C) - p(E|-C) = 1 - p(A) = 1 - p(A)
...
df.pred <- expand_grid(p_C, p_A, structure, method='derived') %>%
    mutate(N=max(n),                  ## number of CF samples
           K=ifelse(structure=='Conjunctive', p_A, 1-p_A), ## causal strength estimate
           K.var=p_A*(1-pA),
           K.sd=sqrt(K.var),
           K.cv=K.sd/K,
